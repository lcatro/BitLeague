...
function captcha_click() {
    ...
    img_captcha.src = 'aurora/captcha_picture/loading.gif?' + Math.random();
    captcha_calculate_worker = new Worker('aurora/captcha/aurora_worker.js');
    
    captcha_calculate_worker.onmessage = function (message) {
        img_captcha = document.getElementById('captcha');
        
        img_captcha.src = 'aurora/captcha_picture/end.png?' + Math.random();
